...
## 7. SSH Configuration Troubleshooting and Override Resolution
...
The following configuration was updated in the main SSH daemon configuration file:
```bash
sudo nano /etc/ssh/sshd_config
```
**The intended setting was:**
PasswordAuthentication no

### Issue Observed

Despite this change, password-based login was still possible. This indicated that the expected SSH configuration was not being fully enforced.

### Root Cause

Further investigation revealed that additional SSH configuration files were being loaded from:
```bash
/etc/ssh/sshd_config.d/
```
One or more of these included files were overriding the password authentication setting.
...
```
### Resolution
The override configuration was identified and updated to disable password authentication across the effective SSH configuration fully.

### Security Benefit
This ensured that the VM enforced the intended access control model:

- SSH key-based authentication only
- No remote password-based login

### Key Learning
On Linux cloud instances, security settings may be distributed across multiple configuration files. Effective hardening requires validating the final applied configuration, not just editing the primary config file.

---

## 8. Post-Hardening Access Validation Issue
After updating Azure NSG rules and SSH authentication settings, an SSH connection attempt resulted in a temporary access issue.

### Validation Test
A connection attempt was made from the client device after applying the access hardening controls.
...
### Result Observed
**ssh: connect to host 20.189.74.46 port 22: Connection refused**

### Technical Interpretation
This result indicated that the SSH service or port availability was temporarily unavailable at the time of testing. This can occur after SSH configuration changes, service restarts, firewall rule updates, or network access control modifications.

### Troubleshooting Considerations
The following components were reviewed as part of troubleshooting:

- SSH service status on the VM
- UFW firewall rules
- Azure NSG inbound rules
- Effective SSH daemon configuration

### Security Learning
This highlighted an important operational security principle:
- Hardening changes must always be validated carefully to ensure security controls do not unintentionally disrupt legitimate administrative access.
